...
private:
	...
	void TeleopPeriodic()
	{
		...
		if(limitSwitch->Get() || rightY > 0 || firstPress == false) {
			limitcoder = limitcoder + 1;
			firstPress = true;
		} else if(limitSwitch->Get() || rightY <= 0 || firstPress == false) {
			limitcoder = limitcoder - 1;
			firstPress = true;
		...
		if(driveStick->GetRawButton(6)) {
			...
			if (limitcoder > 0 || limitcoder < 15) {
				lifter->Set((rightY)/1.5);
			} else if(limitcoder >= 15 || rightY <0) { //asdfghjkl;'
				lifter->Set((rightY)/1.5);
			} else if(limitcoder <= 0 || rightY>15) {
				lifter->Set((rightY)/1.5);
			} else {
				lifter->Set(0);
			}
